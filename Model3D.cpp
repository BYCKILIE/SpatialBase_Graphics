...
namespace gps {
	...
	glm::vec3 calculateCenter(const std::vector<glm::vec3>& vertices) {
	...
	glm::vec3 Model3D::getModelCenter() const {
		return modelCenter;
	}
	...
	void Model3D::ReadOBJ(std::string fileName, std::string basePath) {
		...
		for (size_t s = 0; s < shapes.size(); s++) {
			...
			for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
				...
				for (size_t v = 0; v < fv; v++) {
                    ...
                    centers.push_back(vertexPosition);
		...
		modelCenter = calculateCenter(centers);
	}
